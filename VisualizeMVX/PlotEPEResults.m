...
%%%% ffmpeg without MV spatial interpolation
EPE_JM_AVLib_MVSIntp=[59.8979 60.0349 61.3386 61.5942 54.7735 52.6651 51.1620 47.0293 45.1994 43.5351   41.9629   39.4574   32.8244   27.2022];

EPE_JM_QP0_MVSIntp=  [0       27.9198 33.0515 37.9323 41.9253 48.0689 54.8066 59.0676 64.9660 70.0853 73.3833 75.6284 72.2681 69.8839];

EPE_JM_QP12_MVSIntp= [37.9323 39.1497 38.0940 0       43.8922 51.4667 55.5073 60.3693 66.2299 71.4165 75.8124 77.8443 74.2889 71.4300];
...
figure()
plot(QP,EPE_JM_AVLib_MVSIntp,'-r*');
hold on;
plot(QP,EPE_JM_QP0_MVSIntp,'-b>');
plot(QP,EPE_JM_QP12_MVSIntp,'-kd');
plot(QP,EPE_JM_QP51_MVSIntp,'-go');
title('EPE for MV extracted from JM coded Videos with MV spatial Interpolation');
xlabel('QP values');
ylabel('EPE');
grid on;
legend('AVLib MVs (used to train 3DCNN)','JM MV with QP=0 (Lossless)','JM MV with QP=12','JM MV with QP=51');